	...
	public void saveToDB(){
		if(this.id==0){
			try {
				...
				PreparedStatement stmt = DbUtil.getConn().prepareStatement("INSERT INTO user_group(name) VALUES (?)",generatedColumns);
			...
			try{
				PreparedStatement stmt = DbUtil.getConn().prepareStatement("UPDATE user_group SET name = ? WHERE id = ?");
				stmt.setString(1, this.name);  
