...
public class Solution {
	...
	public Solution(Date created, Date updated, String description){
		this.id = 0;
//		this.setCreated(created).setUpdated(updated).setDescription(description);
	}
	...
	public void saveToDB(){
		if(this.id==0){
			try {
				...
				PreparedStatement stmt = DbUtil.getConn().prepareStatement("INSERT INTO solution(created,updated,description) VALUES (?,?,?)",generatedColumns);
			...
			try{
				PreparedStatement stmt = DbUtil.getConn().prepareStatement("UPDATE solution SET created = ?,updated = ?,description = ?, WHERE id = ?");
				stmt.setDate(1, (java.sql.Date) this.created); 
				stmt.setDate(2, (java.sql.Date) this.updated);
				stmt.setString(3, description);
				stmt.setInt(3, id);
