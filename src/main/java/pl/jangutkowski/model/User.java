	...
	public void saveToDB(){
		if(this.id==0){
			try {
				...
				PreparedStatement stmt = DbUtil.getConn().prepareStatement("INSERT INTO users(username,email,password,salt,person_group_id) VALUES (?,?,?,?,?)",generatedColumns);
			...
			try{
				PreparedStatement stmt = DbUtil.getConn().prepareStatement("UPDATE users SET username=?, email=?, person_group_id=?, password=?, salt=? WHERE id=?");
				stmt.setString(1, this.username);
				stmt.setString(2, this.email);
...
//				System.out.println(stmt);
	...
	// static DB methods
	public static ArrayList<User> loadAllByGroupId(int groupId) throws SQLException {
	...
	public static ArrayList<User> loadAll() throws SQLException {
		String sql = "SELECT * FROM users"; 
		PreparedStatement stmt = DbUtil.getConn().prepareStatement(sql);
		return getUsersFromStatement(stmt);
	}
